...
type Props = {
  /** Montant cumulé réel (en €). Si non fourni, fallback au mock. */
  spend?: number
}
...
export default function TiersShowcase({ spend: spendProp }: Props) {
  ...
  const remaining = Number((next ? Math.max(0, next.min - spend) : 0).toFixed(2))
    ...
    <section className="mt-8">
      ...
      <div className="mt-4 rounded-xl border border-border p-4 bg-white/60 dark:bg-slate-900/40">
        <div className="flex items-center justify-between text-sm">
          ...
          <span className="text-muted">Total dépensé : {spend.toFixed(2)}€</span>
        </div>
